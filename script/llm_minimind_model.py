...
def precompute_freqs_cis(dim: int, end: int, rope_base: float, rope_scaling: Optional[dict] = None):
    ...
    if rope_scaling is not None:
        ...
        if end / orig_max > 1.0:
            ...
            ramp = torch.clamp((torch.arange(dim // 2, device=freqs.device).float() - low) / max(high - low, 1e-3), 0,
                               1)
...
class Attention(nn.Module):
    def __init__(self, cfg: MiniMindConfig):
        super().__init__()
        kvh = cfg.num_attention_heads if cfg.num_key_value_heads is None else cfg.num_key_value_heads
    ...
    def forward(
            ...
            past_key_value: Optional[Tuple[torch.Tensor, torch.Tensor]] = None,  # (k,v) each (B,T,KVH,D)
        ...
        if self.flash and (s > 1) and (past_key_value is None) and (
...
class MoEGate(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.cfg.scoring_func != "softmax":
            raise NotImplementedError(f"Unsupported scoring_func: {self.cfg.scoring_func}")
        ...
        if self.training and self.alpha > 0.0:
            if self.seq_aux:
                ...
                ce.scatter_add_(1, topi_b, torch.ones(b, s * self.top_k, device=x.device))
...
class MOEFeedForward(nn.Module):
    def __init__(self, cfg: MiniMindConfig):
        super().__init__()
        self.cfg = cfg
        self.experts = nn.ModuleList([FeedForward(cfg) for _ in range(cfg.n_routed_experts)])
        self.gate = MoEGate(cfg)
        self.shared = nn.ModuleList(
            [FeedForward(cfg) for _ in range(cfg.n_shared_experts)]) if cfg.n_shared_experts > 0 else None
        self.aux_loss = None
...
class MiniMindBlock(nn.Module):
    ...
    def forward(self, x, pos_emb, past_kv=None, use_cache=False, attention_mask=None):
        r = x
        x, present = self.self_attn(self.input_layernorm(x), pos_emb, past_kv, use_cache, attention_mask)
        x = x + r
...
class MiniMindModel(nn.Module):
    def __init__(self, cfg: MiniMindConfig):
        ...
        self.dropout = nn.Dropout(cfg.dropout)
        self.layers = nn.ModuleList([MiniMindBlock(i, cfg) for i in range(cfg.num_hidden_layers)])
    ...
    def forward(self, input_ids, attention_mask=None, past_key_values=None, use_cache=False, **_):
        b, s = input_ids.shape
        ...
        pos_emb = (self.freqs_cos[start_pos:start_pos + s], self.freqs_sin[start_pos:start_pos + s])
        ...
        x = self.norm(x)
        aux = sum((l.mlp.aux_loss for l in self.layers if isinstance(l.mlp, MOEFeedForward)), x.new_zeros(()))
...
class MiniMindForCausalLM(PreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        sel = slice(-logits_to_keep, None) if isinstance(logits_to_keep, int) else logits_to_keep
        ...
        out = CausalLMOutputWithPast(loss=loss, logits=logits, past_key_values=pkv, hidden_states=hs)
        out.aux_loss = aux
        return out
