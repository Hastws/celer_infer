/**
 * DiT Extreme-Optimized Inference Engine
 * OpenMP + AVX2 + Cache-Optimized Tiling
 */
...
// ============================================================================
constexpr int TILE_SIZE = 64;      // L1 cache friendly
constexpr int BLOCK_SIZE = 256;    // L2 cache friendly
...
inline __m256 gelu_avx(__m256 x) {
    __m256 c1 = _mm256_set1_ps(0.7978845608f);
...
// ============================================================================
// Configuration & Weight Structures
// ============================================================================
struct DiTConfig {
    int future_len, time_len, agent_num, static_objects_num, lane_num;
    int encoder_depth, decoder_depth, num_heads, hidden_dim;
...
struct MultiHeadAttentionWeights {
    ...
    int embed_dim = 0, num_heads = 0;
};

struct MixerBlockWeights {
    LayerNormWeights norm1, norm2;
    MLPWeights channels_mlp, tokens_mlp;
};
...
struct DiTBlockWeights {
    LayerNormWeights norm1, norm2, norm3, norm4;
    MultiHeadAttentionWeights attn, cross_attn;
    MLPWeights mlp1, mlp2;
    LinearWeights adaLN_modulation;
};

struct FinalLayerWeights {
    LayerNormWeights norm_final, proj_norm1, proj_norm2;
    LinearWeights proj_linear1, proj_linear2, adaLN_modulation;
};

struct DiTWeights {
    LinearWeights encoder_pos_emb, neighbor_type_emb;
    MLPWeights neighbor_channel_pre, neighbor_token_pre;
    std::vector<MixerBlockWeights> neighbor_blocks;
    LayerNormWeights neighbor_norm;
    MLPWeights neighbor_emb_project, static_projection;
    LinearWeights lane_speed_limit_emb, lane_traffic_emb;
    const float* lane_unknown_speed_emb = nullptr;
    MLPWeights lane_channel_pre, lane_token_pre;
...
// ============================================================================
// Extreme Operations with OpenMP + SIMD + Tiling
// ============================================================================
namespace ops {

// Tiled matrix multiply with OpenMP
...
void layer_norm_omp(float* out, const float* x, const float* weight, const float* bias,
                    int batch, int dim) {
...
void linear_omp(float* out, const float* x, const LinearWeights& w, int batch) {
...
void mlp_gelu_omp(float* out, const float* x, const MLPWeights& w, int batch, float* tmp) {
    int hidden = w.fc1.out_features;
    
    linear_omp(tmp, x, w.fc1, batch);
    ...
    linear_omp(out, tmp, w.fc2, batch);
}

void add_vectors_omp(float* a, const float* b, int n) {
    #pragma omp parallel for simd
    for (int i = 0; i < n; i += 8) {
        if (i + 8 <= n) {
            __m256 av = _mm256_loadu_ps(a + i);
            __m256 bv = _mm256_loadu_ps(b + i);
            _mm256_storeu_ps(a + i, _mm256_add_ps(av, bv));
        }
    }
...
// ============================================================================
// JSON Loading (same as SIMD version)
// ============================================================================
...
public:
    ...
    LinearWeights load_linear(const json& j) {
        ...
        if (j.contains("bias")) w.bias = load_tensor(j["bias"]);
        return w;
    }
    ...
    DiTWeights load_weights(const json& weights_json, const DiTConfig& cfg) {
        ...
        for (auto& blk : ne["blocks"]) w.neighbor_blocks.push_back(load_mixer_block(blk));
        ...
        for (auto& blk : le["blocks"]) w.lane_blocks.push_back(load_mixer_block(blk));
        w.lane_norm = load_layernorm(le["norm"]);
        w.lane_emb_project = load_mlp(le["emb_project"]);
        
        auto& fe = weights_json["fusion_encoder"];
        for (auto& blk : fe["blocks"]) w.fusion_blocks.push_back(load_self_attn_block(blk, cfg.num_heads));
        ...
        for (auto& blk : weights_json["dit_blocks"]) w.dit_blocks.push_back(load_dit_block(blk, cfg.num_heads));
...
// ============================================================================
// DiT Inference (OpenMP parallelized)
// ============================================================================
class DiTInference {
public:
    DiTConfig cfg;
    ...
    void timestep_embedding(float* out, const float* t, int batch) {
        ...
        ops::linear_omp(tmp1.data(), out, weights.t_embedder_mlp_0, batch);
        ...
        ops::linear_omp(out, tmp1.data(), weights.t_embedder_mlp_2, batch);
    }
    ...
    void encoder_forward(float* encoding, const float* inputs, int batch) {
        ...
        for (auto& block : weights.fusion_blocks) {
            ops::layer_norm_omp(tmp1.data(), encoding, block.norm1.weight, block.norm1.bias,
                               batch * token_num, cfg.hidden_dim);
            ops::layer_norm_omp(tmp2.data(), encoding, block.norm2.weight, block.norm2.bias,
                               batch * token_num, cfg.hidden_dim);
            ops::mlp_gelu_omp(tmp3.data(), tmp2.data(), block.mlp, batch * token_num, tmp4.data());
            ops::add_vectors_omp(encoding, tmp3.data(), batch * token_num * cfg.hidden_dim);
        }
        
        ops::layer_norm_omp(tmp1.data(), encoding, weights.fusion_norm.weight, weights.fusion_norm.bias,
                          batch * token_num, cfg.hidden_dim);
        std::copy(tmp1.begin(), tmp1.begin() + batch * token_num * cfg.hidden_dim, encoding);
    }
    
    void decoder_forward(float* score, const float* x, const float* t, const float* encoding, int batch) {
        int P = 1 + cfg.predicted_neighbor_num;
        int output_dim = (cfg.future_len + 1) * 4;
        
        ops::mlp_gelu_omp(tmp1.data(), x, weights.preproj, batch * P, tmp2.data());
        ...
        for (auto& block : weights.dit_blocks) {
            ...
            ops::linear_omp(modulation.data(), y.data(), block.adaLN_modulation, batch);
            
            ops::layer_norm_omp(tmp2.data(), current, block.norm1.weight, block.norm1.bias,
                              batch * P, cfg.hidden_dim);
            ops::layer_norm_omp(tmp3.data(), tmp2.data(), block.norm2.weight, block.norm2.bias,
                              batch * P, cfg.hidden_dim);
            ops::mlp_gelu_omp(tmp4.data(), tmp3.data(), block.mlp1, batch * P, tmp2.data());
            
            ops::layer_norm_omp(tmp2.data(), tmp4.data(), block.norm3.weight, block.norm3.bias,
                              batch * P, cfg.hidden_dim);
            ops::layer_norm_omp(tmp3.data(), tmp2.data(), block.norm4.weight, block.norm4.bias,
                              batch * P, cfg.hidden_dim);
            ops::mlp_gelu_omp(current, tmp3.data(), block.mlp2, batch * P, tmp4.data());
        }
        
        ops::layer_norm_omp(tmp2.data(), current, weights.final_layer.norm_final.weight,
                          weights.final_layer.norm_final.bias, batch * P, cfg.hidden_dim);
        ops::layer_norm_omp(tmp3.data(), tmp2.data(), weights.final_layer.proj_norm1.weight,
                          weights.final_layer.proj_norm1.bias, batch * P, cfg.hidden_dim);
        ops::linear_omp(tmp4.data(), tmp3.data(), weights.final_layer.proj_linear1, batch * P);
        
        int total_out = batch * P * weights.final_layer.proj_linear1.out_features;
        #pragma omp parallel for simd
        for (int i = 0; i < total_out; i += 8) {
            if (i + 8 <= total_out) {
                __m256 v = _mm256_loadu_ps(tmp4.data() + i);
                _mm256_storeu_ps(tmp4.data() + i, simd::gelu_avx(v));
            }
        }
        for (int i = (total_out / 8) * 8; i < total_out; ++i) {
            float x_val = tmp4[i];
            float x3 = x_val * x_val * x_val;
            float inner = 0.7978845608f * (x_val + 0.044715f * x3);
        ...
        ops::layer_norm_omp(tmp2.data(), tmp4.data(), weights.final_layer.proj_norm2.weight,
                          weights.final_layer.proj_norm2.bias, batch * P, weights.final_layer.proj_linear1.out_features);
        ops::linear_omp(score, tmp2.data(), weights.final_layer.proj_linear2, batch * P);
    }
};
...
int main(int argc, char** argv) {
    ...
    std::cout << "[EXTREME] OpenMP threads: " << num_threads << std::endl;
    ...
    std::cout << "[EXTREME] Config: hidden=" << cfg.hidden_dim << " depth=" << cfg.encoder_depth 
    ...
    std::cout << "========================================" << std::endl;
    std::cout << "[EXTREME Forward] Score shape: (" << B << ", " << P << ", " << output_dim << ")" << std::endl;
